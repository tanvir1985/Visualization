...
x = np.loadtxt("position.txt",  skiprows=39742)[:,0]
y = np.loadtxt("position.txt",  skiprows=39742)[:,1]
z = np.loadtxt("position.txt",  skiprows=39742)[:,2]
...
fig.update_layout(scene=dict(aspectratio=dict(x=1.5, y=1.5, z=0.5),xaxis = dict(range=[x_min,x_max],),yaxis = dict(range=[y_min,y_max])))
# fig.write_image("F:/aug21/interface_different radius/contact line fluc/fig1.png", width=1920, height=1080)
